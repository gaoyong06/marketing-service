...
type CouponRepo interface {
	...
	UseCoupon(context.Context, string, uint64, string, string, int64, int64, int64) error // 使用优惠券（事务操作）：code, userID, paymentOrderID, paymentID, originalAmount, discountAmount, finalAmount
...
func (uc *CouponUseCase) Use(ctx context.Context, code string, userID uint64, paymentOrderID, paymentID string, originalAmount, discountAmount, finalAmount int64) error {
	// 使用事务确保原子性：先增加使用次数，再创建使用记录
	// 如果创建使用记录失败，需要回滚使用次数的增加
	// 注意：这里依赖 Repository 层的事务支持，如果 Repository 不支持事务，需要在 UseCase 层实现
	return uc.repo.UseCoupon(ctx, code, userID, paymentOrderID, paymentID, originalAmount, discountAmount, finalAmount)
}
