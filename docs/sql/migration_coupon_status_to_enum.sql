...
-- 状态值：active(激活-可使用)/inactive(停用-不可使用)/expired(已过期-系统自动标记)
...
-- 步骤2: 修改字段类型为 enum
ALTER TABLE `coupon`
MODIFY COLUMN `status` enum('active','inactive','expired') NOT NULL DEFAULT 'active' COMMENT '优惠券状态: active(激活-可使用)/inactive(停用-不可使用)/expired(已过期-系统自动标记)';
